...
export default function Height() {
  ...
  const heights = Array.from({ length: 121 }, (_, i) => i + 75); // cm: 60 to 180
  const [selected, setSelected] = useState(170); // Default height

  const flatListRef = useRef<FlatList>(null);

  const handleContinue = () => {
    console.log('Selected Height:', selected, 'cm');
    router.push('/beforeHome/getStart/remainderTime');
  };
      ...
      <View style={styles.container}>
        ...
        <View style={styles.pickerWrapper}>
          <FlatList
            ...
            initialScrollIndex={selected - heights[0]}
            renderItem={({ item }) => {
...
const styles = StyleSheet.create({
  ...
  indicator: {
    position: 'absolute',
    top: (PICKER_HEIGHT + hp('1%') - ITEM_HEIGHT) / 2,
    height: ITEM_HEIGHT,
    width: '55%',
    borderTopWidth: 2,
    borderBottomWidth: 2,
